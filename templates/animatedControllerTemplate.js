/**
 *	The template for new Animated Controllers.
 *	Animated controllers can draw whenever the objects need
 *	to be drawn, even if there is no interactive event to trigger
 *	the redraw.  This controller is also interactive and can
 *	respond to user events.
 * 
 *	Animated controllers use a ticking timer to handle
 *	all drawing, so it is the most performance heavy, although
 *	the cost is still small.  This is the recommended template
 *	for most controllers, since it has the full functionality built
 *	in from the start.
 *
 *	You should start with this template if you are trying to
 *	create a component that has animated elements, or if you
 *	are familiar with the framework and want the most complete
 *	template as a base for your controller.
 *
...
/*
 * The function that controls the animation loop.
 * This method runs at 1000/timeout times per second,
 * evaluating if the canvas or objects need to be changed,
 * and requesting the next frame.  Note that it will NOT
 * draw the canvas every frame unless an object has requested
 * a redraw with the requestRedraw() method.  
 * 
 * This function is also the place to put code that needs to
 * happen every frame, like ticking objects or a counter.
 */
this.drawingLoop = function()
{
	/*
		Insert code (if any)
		that should be called upon
		entering a new frame.
	*/	
	
	if(!this.running){return;}
	this.thread = torch.drawController(this, this.timeout);
	
	if(this.canvas.redrawNeeded)
	{
		this.context.clearRect(0,0,this.canvas.width,this.canvas.height);	   
		this.draw();
		this.canvas.oncanvasdrawn();
	}
	
	/*
		tick() your objects here,
		if necessary.
	*/
};
...
/*
 * Starts the controller's animation and interaction.
 */
this.play = function()
{
	this.running = true;
...
/*
 * Pauses this controller so that it stops
 * the drawingLoop.
